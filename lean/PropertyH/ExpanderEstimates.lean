...
Lipschitz constant has already been constructed. No cardinality assumption is
needed; for an empty vertex set the left-hand side is zero in Lean.
...
theorem expander_poincare {V : Type*} [Fintype V] (G : SimpleGraph V)
  ...
  exact expander_poincare_proved G k h hG μ f L hL hf
